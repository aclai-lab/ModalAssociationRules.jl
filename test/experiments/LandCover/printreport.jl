...
function printreport(
    _miner::Miner, i::Int, rules::Vector{ARule}; reportprefix::String="results_"
)
    # we expect the experiment to consider global confidence and global lift
    rulecollection = [
        (
            rule,
            round(globalmemo(_miner, (:gsupport, antecedent(rule))); digits=2),
            round(globalmemo(_miner, (:gsupport, Itemset(rule))); digits=2),
            round(globalmemo(_miner, (:gconfidence, rule)); digits=2),
            round(globalmemo(_miner, (:glift, rule)); digits=2),
        ) for rule in rules
    ]
    ...
    open(reportname, "w") do io
        ...
        padding = maximum(length.(freqitems(_miner)))
